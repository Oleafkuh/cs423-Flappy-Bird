...
/**
 * Saves a guest score entry (no account needed) to localStorage.
 * @param {string} name - The generated rapper name.
 * @param {number} score - The score to save.
 */
export function saveGuestScore(name, score) {
  const entries = JSON.parse(localStorage.getItem("flappyGuestLeaderboard") || "[]");
  entries.push({ username: name, score: score, date: new Date().toISOString() });
  entries.sort((a, b) => b.score - a.score);
  localStorage.setItem("flappyGuestLeaderboard", JSON.stringify(entries.slice(0, 50)));
}

/**
 * Gets the guest leaderboard (top 10, no account required).
 * @returns {Array} Array of { username, score, date } entries.
 */
